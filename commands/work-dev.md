...
3. **复盘提炼**:默认走 §11 三行(问题 / 方案 / 沉淀?),不展开 retro;用户喊 `retro` 才填完整复盘段(也仅 3 题:坑 / 根因 / 避免)。

   **沉淀判定四问**(全部 yes 才进 lessons,任一 no 则**不沉淀**):
   - **a. 跨任务共性?** 同类问题是否在**别的任务**里也会出现?(项目特异的代码 quirk → 写到 `{docs_root}/<category>/<module>.md`,不进 lessons)
   - **b. 可重现?** 是否能预期下次某场景必触发?(纯偶发 / 一次性 → 不沉淀)
   - **c. 可操作?** 能写成"做 X" / "不做 X" 的祈使句?(只能描述现象、无法给操作指令 → 不沉淀)
   - **d. 非已有?** lessons 中无同类条目?(已有 → 仅触发次数 +1,不新增)

   **写入规则**:
   - 四问全 yes → 追加到对应模块段(`### 通用陷阱` / `### bug 修复类` / `### 死代码清理类` / `### Phase 流程` 等);格式 `- <祈使句一句话规则>(来源: <task 文件>;触发次数: 1)`
   - 命中已有同类(第 4 问 no) → 触发次数 +1;触发次数 ≥3 自动移到顶部「高频坑」段
   - 其它情况 → **跳过沉淀**,§11 "沉淀?" 标 `no`,留作任务文档内的私有记录即可

   **强约束**:每个任务最多沉淀 **1 条** 新规则;凑不出 1 条就 0 条,**不强求凑数**。
   §11 "追加到 _lessons.md 的条目"小节仅在实际写入 lessons 时回填,否则留空。
4. **落盘第 10/11 节**:第 10 节(变更记录,本次一次性写完,不含 commit hash);第 11 节(默认三行 + 实际写入的 lessons 条目;`retro` 模式额外展开完整复盘 3 题);frontmatter `phase: 7` / `status: done` / `last-updated`。
