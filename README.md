...
如果默认值不够,可以创建 `.work-dev/config.jsonc` 显式 override(详见 §6)。
...
本章面向**第一次用 cc-work-dev 的用户**,从触发命令到 commit 收口的完整链路。`/work-dev init` 之后,日常使用就是本章 §4.2 的循环。

### 4.1 三种触发方式

| 方式 | 输入 | 适用场景 |
|---|---|---|
| **显式命令(推荐)** | `/work-dev <complexity> <描述>` | 明确知道任务规模时 |
| **省略复杂度** | `/work-dev <描述>` | 让 plugin 走 `default_complexity`(默认 standard) |
| **自然语言触发** | "帮我修 X bug" / "实现 Y 功能" 等 | SKILL.md description 语义命中,自动调起本流程 |
| **中断后继续** | "继续" / "接着做" | Claude 读最新任务文档 frontmatter 的 `phase` + `status` 自动定位 |

### 4.2 完整 walkthrough — 以"修 auth 模块 token 失效 bug"为例

下面是一个 **standard** 复杂度任务的端到端时序。每行 `→` 表示用户输入,`←` 表示 Claude 输出。

```
[Phase 1 — Discovery]                              耗时:30 秒
→ 用户:/work-dev standard 修 auth 模块 token 失效后未跳转登录页的 bug
← Claude:[work-dev v3 / standard / Phase 1]
         复述: 你想修复 token 失效后页面卡住,为了用户重新登录,
              约束 不破坏正常会话续期
         类型: bug | 模块: feature/auth | 复杂度: standard
         召回历史坑: 命中 2 条(见 _lessons.md feature/auth 段)
         已创建 tasks/2026-05-21-1430-auth-fix-token-redirect.md
         (frontmatter: status=draft / phase=1)

[Phase 2 — Explore]                                耗时:1-2 分钟
← Claude:[Phase 2] 启动 2 个 explorer agent 并行...
         agent A 返回: 5 个关键文件 + 调用链
         agent B 返回: 3 个集成点 + 跨端一致性分析
         整合: 5-Why 根因 / 5 维影响面 / 高风险标注
         已落盘 §2/§3/§4

[Phase 3 — Clarify](可能跳过)                     耗时:0-1 分钟
← Claude:[Phase 3] 检测到 2 个澄清点
         AskUserQuestion 弹出枚举:
           Q1: token 失效检测时机? [响应拦截 / 定时轮询 / 两者]
           Q2: 已登录用户怎么处理草稿表单? [丢弃 / 保留 30 天 / 询问]
→ 用户:[响应拦截] [保留 30 天]
← Claude:已落盘 §5;如答案影响根因则回 Phase 2 步骤 3 更新

[Phase 4 — Design](⚠️ 必答节点 1/2)               耗时:1-2 分钟
← Claude:[Phase 4] 启动 2 个 architect 并行(compact mode)
         A — minimal-diff:在 axios 拦截器加跳转
         B — refactor-with-feature:抽 useAuthGuard hook
         推荐: A(改动 12 行 vs B 改动 80 行,本任务紧急)
         已落盘 §6/§7/§8
         AskUserQuestion 弹: [开干 A] [开干 B] [改方案] [取消]
→ 用户:[开干 A]
← Claude:frontmatter status=in-progress,进 Phase 5

[Phase 5 — Implement]                              耗时:取决于任务
← Claude:[Phase 5] 按 §7 拆解步骤实施
         step 1: 改 src/auth/interceptor.ts:42 ✓
         step 2: 加 src/auth/redirect.ts ✓
         step 3: 改 src/router/guards.ts:18 ✓
         不 commit,改动留 working tree
         self-check: §7 全 [x],§8 留 Phase 6 跑测后勾

[Phase 6 — Review](⚠️ 必答节点 2/2)               耗时:1-3 分钟
← Claude:[Phase 6] 跑测试:npm test
         结果: 23 pass / 0 fail
         启动 2 个 reviewer agent...
         findings: 无 confidence ≥ 80 issue
         §8 验收勾选: ✅ Happy path ✅ 边界 1 ⚠ 边界 2 (未覆盖)
         AskUserQuestion 弹: [fix now] [fix later] [proceed]
→ 用户:[fix later]   # 我先把这个上线,边界 2 周三再补
← Claude:§10 追加 TODO,进 Phase 7

[Phase 7 — Wrap]                                   耗时:1 分钟
← Claude:[Phase 7] docs 漂移扫描...
         检测到 3 个符号在 docs/features/auth.md 仍是旧描述
         standard → 自动 Edit 同步,再次扫描 → 干净
         §11 复盘三行:
           问题: 拦截器只清了 token 没 push 路由
           方案: 在 401 分支加 router.push('/login')
           沉淀?: yes(对照四问全 yes)
         追加到 _lessons.md feature/auth 段 1 条规则
         一次 commit 收口: fix(auth): redirect to /login on token expiry
         frontmatter status=done

总耗时:约 5-10 分钟(不含 Phase 5 实施时间)
```

### 4.3 各 Phase 用户视角速查表

| Phase | Claude 自动做 | 用户必做 | 用户可选做 |
|---|---|---|---|
| 1 Discovery | 复述需求 / 召回历史坑 / 建任务文档 | 看复述是否准确(不准就反驳) | — |
| 2 Explore | 启 explorer / 5-Why / 5 维影响 | — | 读 §2/§3/§4 觉得遗漏 → 追加 |
| 3 Clarify | 识别澄清点 | **答 AskUserQuestion** (可能 0 题) | — |
| 4 Design | 启 architect / 出方案对比 / 推荐 | **答 AskUserQuestion** 选 A/B/改/取消 | 看 §7 拆解觉得不对 → 选"改方案" |
| 5 Implement | 按 §7 实施 / 不 commit | — | 中途想 commit → 说"边做边 commit" |
| 6 Review | 跑测试 / 启 reviewer / 勾验收 | **答 AskUserQuestion** fix-now/later/proceed | — |
| 7 Wrap | docs 漂移扫描 + 同步 / commit 收口 / 提炼 lessons | — | 喊 `retro` 走完整复盘 3 题 |

**关键纪律**:全程 Claude 仅在 Phase 3/4/6 三处可能问你,Phase 4 + Phase 6 是**必答 2 节点**,其他用文字段输出 + 隐式确认(你不反驳就视为同意)。

### 4.4 决策节点详解 — 只有 2 次必答

#### 节点 1/2:Phase 4 末「选方案」

| 选项 | 含义 | 后续 |
|---|---|---|
| `开干 A` | 采纳方案 A | 进 Phase 5,frontmatter `status: in-progress` |
| `开干 B` | 采纳方案 B(deep 还会有 C) | 同上 |
| `改方案` | 都不满意 | 回 Phase 3 补澄清,或回 Phase 4 步骤 1 重启 architect;§5/§6/§7/§8 覆盖更新(不开派生文档) |
| `取消` | 任务作废 | `status: cancelled`,§10 追加理由 |

#### 节点 2/2:Phase 6 末「fix-then-recheck」

| 选项 | 含义 | 后续 |
|---|---|---|
| `fix now` | 当前 finding 立刻修 | 修 → 重跑测试 → 改动 >20 行或高风险时重起 reviewer → 再问一次 |
| `fix later` | 留 TODO 走 Phase 7 | §10 追加 TODO,进 Phase 7 |
| `proceed` | 直接收口 | 进 Phase 7 |

### 4.5 中断后如何继续

cc-work-dev 设计成**意外中断不丢工作**:每个 Phase 完成立刻 Edit 任务文档对应章节 + frontmatter 同步 `phase` / `status` / `last-updated`。

恢复方法:

```
→ 用户:继续        # 或 "接着上次的做" / "断点续跑"
← Claude:读 {tasks_root}/ 下 last-updated 最新且 status ∉ {done, cancelled} 的任务文档
         读 frontmatter 的 phase 和 status
         从该 Phase 的下一步继续,不从头跑
```

**典型场景**:
- 关机重启 → 任务文档磁盘上完整,下次启动 Claude Code 输"继续"即可
- Phase 5 实施到一半网络中断 → §7 已勾的步骤 `[x]`,未勾的 `[ ]`,从第一个 `[ ]` 继续
- 不小心关了 Claude Code → 同上

### 4.6 复杂度判定速查

不确定填 light/standard/deep?对照下表:

| 信号 | 推荐档 |
|---|---|
| typo / 单行改 / 重命名 / 改注释 | **light** |
| <10 行 / 单文件 / 无新公开 API | **light** |
| 多文件 / 新功能 / 公开行为变更 / 默认场景 | **standard** |
| DB schema / 认证流程 / 支付逻辑 / 跨 ≥3 端 / 删文件表字段 | **deep** |

**做错也能改**:Phase 2 实测后发现低估/高估,Phase 5 之前都可以升降(详见 commands/work-dev.md §9 "中途升级与降级")。

### 4.7 配置调优(可选)

零配置可跑;有以下场景再写 `.work-dev/config.jsonc`(全字段详见 §6):

| 场景 | 写哪个字段 |
|---|---|
| 项目用 `documentation/` 不用 `docs/` | `docs_root` |
| monorepo,代码在 `packages/` | `code_roots: ["packages/"]` |
| 多端测试命令不同 | `test_commands: { backend: ..., web: ... }` |
| 想严格防止新模块自动建占位 | `strict_modules: true` |
| 项目有自己的高风险类型(如"微信激活码兑换") | `high_risk_ops_extra: [...]` |
| 不希望 Phase 7 自动 commit | `auto_commit: false` |
| 想减少 agent 数量(资源敏感) | `agent_counts.standard: { explorer: 1, ... }` |

### 4.8 常见陷阱(避坑指南)

| 陷阱 | 现象 | 怎么避免 |
|---|---|---|
| **复杂度估错** | light 任务做着发现要改 5 个文件 | Phase 2 explorer 出来后看清楚再批准 Phase 4;实测和估算差很多就升降复杂度 |
...
### 8.2 `work-dev-architect`(Phase 4)
