...
|---|---|---:|---|
| **light** | <10 行 / 单文件 / typo / 重命名 | 4 (§1/4/7/8) | explorer 1 / architect 1 / reviewer 1 |
| **standard**(默认) | 多文件 / 新功能 / 公开行为变更 | 8 (§1-8) | 各 2 |
...
- Phase 7 提炼 1-2 条新规则追加;触发 ≥3 次自动移到顶部「高频坑」段
- 条目格式:`- <一句话规则>(来源: <task 文件>;触发次数: N)`

### 3.5 docs 漂移自动守门

Phase 7 从 `git diff` 提取改动符号(函数 / 类 / 常量 / 配置项 / API 路径 / DB 字段),对每个符号执行:

```bash
grep -rn "<symbol>" {docs_root}/ README.md {drift_scan_extra_targets[]}
```
...
| 重大架构决策 | 新建 `docs/decisions/<YYYY-MM-...>.md` |
...
**多 architect 并行差异化**:standard=2 / deep=3 时,每个 agent 被分配焦点角度(minimal change / clean architecture / pragmatic balance),不允许输出混合方案。
...
`tasks/_lessons.md` 通常 100-300 行级别,长期项目可能膨胀。机制保护:
...
完整设计 ADR(brainstorming 7 题对齐输出):

- `~/Downloads/work_score/docs/decisions/2026-05-20-work-dev-plugin-config-interface.md`(本地路径,M8 发布后改 GitHub 链接)
